...
public class ContactList {
    ...
    public boolean remove(String name) {
        boolean b = false;
        int i = 0;
        while (i <= defaultCapacity - 1) {
            if (name == ContactList[i].getContactName()) {
                while (i < defaultCapacity && i != defaultCapacity - 1) {
                    ContactList[i++] = ContactList[i];
                }
                defaultCapacity--;
                ContactList = Arrays.copyOf(ContactList, defaultCapacity);
                b = true;
                break;
            }
            i++;
